...
metadata:
  cortex-canonical: commands/start-nucleus.md
...
This is a thin host adapter. Starting from this loaded `SKILL.md` file, walk upward to the nearest directory containing `AGENTS.md`, `commands/start-nucleus.md`, and `scripts/cortex_cli.py`; call it `<cortex-repo-root>`. Do not assume the current working directory is the Cortex repository. If no such root is readable, stop.

Read `<cortex-repo-root>/AGENTS.md`, `<cortex-repo-root>/commands/start-nucleus.md`, `<cortex-repo-root>/references/core-contract.md`, and the current host's column (Claude Code, Cowork, Codex, or ChatGPT Work) in `<cortex-repo-root>/references/capability-matrix.md` completely before acting. The command file is the workflow authority. If those sources conflict, stop and report the conflict instead of inventing behavior.

## Invocation

On Claude, use the text following `/start-nucleus`; on Codex, use the text following `$start-nucleus`; in ChatGPT Work, use the user's text after selecting Cortex or this skill with `@`. Natural-language activation is also allowed when the request clearly matches the description.
...
This adapter is intentionally partial. Never imply that its unavailable steps completed. Source access is host-specific and canonical bulk writes are not fully CLI-wired. In ChatGPT Work, preview or skip any step for which the bounded MCP bridge has no mutation tool.
