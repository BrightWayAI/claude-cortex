...
│   └── _retention.md
└── memory/
    ├── index.md              # Auto-maintained catalog of every node (v4.5+); Obsidian home page
    ├── hot.md                # Rolling 7-day context cache (v4.7+); read first by /recall auto-fire
    ...
    ├── archive/              # Archived nodes (memory-level, distinct from <config-root>/archive/ source layer)
...
`memory/index.md` is auto-maintained (zero-LLM, deterministic). It is the canonical entry point for Obsidian users and non-cortex agents. Regenerated by `/end-day` Step 5.5, `/cleanup` Step 4.5, and explicit `/reindex`. See `references/memory-index.md`.

`memory/hot.md` is the rolling 7-day context cache (v4.7+). Loaded first by `/recall` auto-fire at conversation start so every session opens warm. Maintained by `/listen` (nightly), `/morning` (after merge), and `/end-day` Step 5.6. See `references/hot-cache.md`.
